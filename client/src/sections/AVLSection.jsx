...
export default function AVLSection({ selectedAlgo, avlRoot, avlFrame, avlInput, setAvlInput, avlRunning, avlDone, avlGenRef, speed, onSpeedChange, onRun, onStart, onPause, onStep, onReset }) {
  return (
    <>
      <AVLVisualizer
      ...
      <div className="shrink-0 bg-zinc-900 border-t border-zinc-800/50 px-5 py-3 flex flex-wrap items-center gap-3">
        <input
          ...
          min={1} max={999} disabled={avlRunning}
        />
        <button onClick={onRun} disabled={avlRunning || !avlInput.trim()}
        ...
        <button onClick={onStep} disabled={avlRunning || !avlGenRef.current || avlDone}
        ...
        <div className="ml-auto text-[11px] text-zinc-500 uppercase tracking-wider font-semibold">
          Tree: <span className="text-zinc-300 font-mono font-medium">{countNodes(avlRoot)}</span> nodes
        </div>
      </div>
    </>
  );
