...
const OP_LABEL = { rbtInsert: 'Insert', rbtDelete: 'Delete', rbtSearch: 'Search' };

export default function RBTSection({ selectedAlgo, rbtRoot, rbtFrame, rbtInput, setRbtInput, rbtRunning, rbtDone, rbtGenRef, speed, onSpeedChange, onRun, onStart, onPause, onStep, onReset }) {
  return (
    <>
      <RBTVisualizer
      ...
      <div className="shrink-0 bg-zinc-900 border-t border-zinc-800/50 px-5 py-3 flex flex-wrap items-center gap-3">
        ...
        />
        <button onClick={onRun} disabled={rbtRunning || !rbtInput.trim()}
        ...
        <button onClick={onStep} disabled={rbtRunning || !rbtGenRef.current || rbtDone}
        ...
        <div className="ml-auto text-[11px] text-zinc-500 uppercase tracking-wider font-semibold">
          Tree: <span className="text-zinc-300 font-mono font-medium">{countNodes(rbtRoot)}</span> nodes
        </div>
      </div>
    </>
  );
